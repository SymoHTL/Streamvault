...
export default function SearchPage() {
  ...
  return (
    <div>
      ...
      {isLoading && <div className="text-muted dark:text-muted-dark">{t('search.searching')}</div>}

      {data && (
        <div>
          {data.movies.length > 0 && (
            <section className="mb-8">
              <h2 className="text-lg 2xl:text-2xl font-semibold mb-4 2xl:mb-6 text-text dark:text-text-dark">{t('search.movies')} ({data.movies.length})</h2>
          ...
          {data.tvShows.length > 0 && (
            <section className="mb-8">
              <h2 className="text-lg 2xl:text-2xl font-semibold mb-4 2xl:mb-6 text-text dark:text-text-dark">{t('search.tvShows')} ({data.tvShows.length})</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 2xl:grid-cols-7 gap-4 2xl:gap-6">
